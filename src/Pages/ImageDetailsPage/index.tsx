                  ...
                  onchain
                      ...
                      .then(data => {
                        const params = {toEmail: reportData.email, imageTitle: reportData.title, creatorNickname: reportData.nickname,
                          phrase: randomPhrase, imageUrl: imageInfo?.imgUrl ,proveUrl:`https://incomparable-monstera-ca2377.netlify.app/profile/prove?=${reportData.nickname}?=${reportData.title}?=${randomPhrase}?=${imageInfo?.imgUrl}` ,thens:() => {
                          console.log('then');
                            setModal(false);
