              ...
              onchain
                ...
                .then(data => {
                    const params = {toEmail: reportData.email, imageTitle: reportData.title, creatorNickname: reportData.nickname,
                        phrase: randomPhrase, imageUrl: reportData.uri, proveUrl: `https://incomparable-monstera-ca2377.netlify.app/profile/prove?=${reportData.nickname}?=${reportData.title}?=${reportData.phrase}?=${reportData.uri}` ,thens:() => {
                            console.log('then');
                            setModal(false);
